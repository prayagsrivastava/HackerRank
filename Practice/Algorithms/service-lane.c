...
{
    int* temp = NULL, *result = NULL;
    int start, end, counter = 1, count = 1, min;
    for (int c=0; c<cases_rows; c++)
    {
        start = cases[c][0];
        end = cases[c][1];
        for(int d=start; d<=end; d++)
        {
            temp = realloc(temp, sizeof(int)*counter);
            temp[counter-1] = width[d];
            counter++;
        }
        counter--;
        min = minimum(temp, counter);
